...
class BigDecimal extends bignumber_1.BigNumber {
    ...
    constructor(value, decimals = 18) {
        switch (typeof value) {
            case 'object':
                super(bignumber_1.BigNumber.constructorGuard, value instanceof bignumber_1.BigNumber ? value.toHexString() : bignumber_1.BigNumber.from(value).toHexString());
                this.decimals = decimals;
                this.value = parseFloat(this.toString()) / Math.pow(10, decimals);
                break;
            case 'string':
                super(bignumber_1.BigNumber.constructorGuard, bignumber_1.BigNumber.from(value).toHexString());
                this.decimals = decimals;
                this.value = parseFloat(this.toString()) / Math.pow(10, decimals);
                break;
            case 'number':
                super(bignumber_1.BigNumber.constructorGuard, bignumber_1.BigNumber.from((value * Math.pow(10, decimals)).toFixed(0)).toHexString());
                this.decimals = decimals;
                this.value = value;
                break;
            default:
    ...
    mul(b, b_decimals = 18) {
        ...
        const result_BN = super.mul(b).div(bignumber_1.BigNumber.from(10).pow(b_BD.decimals));
    ...
    toDecimals(newDecimals = 18) {
        if (newDecimals == this.decimals)
            return new BigDecimal(this, this.decimals);
        if (newDecimals > this.decimals) {
